...
//console.log('l.control.geosearch messageHideDelay: ', that._config.messageHideDelay);
        ...
        L.DomEvent
            ...
            .on(link, 'click', function() {
                ...
                if (L.DomUtil.hasClass(form, 'displayNone')) {
                    ...
                    if (L.DomUtil.hasClass(container, 'displayClose')) {
                        L.DomUtil.removeClass(container, 'displayClose');
                    }
                    L.DomUtil.addClass(container, 'displayOpen'); // unhide form
                    searchbox.focus();

//                    window.clearTimeout(this.delay);
                    that.delay = window.setTimeout( function() {
                        that.cancelSearch();
                    }, that._config.messageHideDelay);
                } else {
                    L.DomUtil.addClass(form, 'displayNone'); // hide form
                    if (L.DomUtil.hasClass(container, 'displayOpen')) {
                        L.DomUtil.removeClass(container, 'displayOpen'); // unhide form
                    }
                    L.DomUtil.addClass(container, 'displayClose'); // hide form
                }
    ...
    geosearch: function (qry) {
      console.error('l.control.geosearch qry: ', qry);
        var that = this;
    ...
    cancelSearch: function() {
        ...
        if (L.DomUtil.hasClass(this._container, 'displayOpen')) {
            L.DomUtil.removeClass(this._container, 'displayOpen'); // unhide form
        }
        L.DomUtil.addClass(this._container, 'displayClose'); // hide form
    ...
    sendRequest: function (provider, url, qry) {
        ...
        window.parseLocation = function (response) {
            ...
            console.error('L.Geosearch results: ', results)
            that._processResults(results, qry);
