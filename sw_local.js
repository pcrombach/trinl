...
self.addEventListener('fetch', evt => {
  ...
  if (fetchEnabled) {
    ...
    if (evt.request.url.includes('backendTrinl')) {
      ...
      if (evt.request.url.includes('/save/') || evt.request.url.includes('/remove/')) {
      //if (evt.request.url.includes('/syncDown/') || evt.request.url.includes('/save/') || evt.request.url.includes('/remove/')) {
        //console.warn('[ServiceWorker] fetch backend SYNCDOWN, SAVE en REMOVE SKIPPED: ', evt.request.method, evt.request.url);
        network_only(evt);
      } else {
        if (evt.request.url.includes('/loadFotos') || evt.request.url.includes('/loadPois')) {
          cacheDataBackend(evt);
        } else {
          if (evt.request.url.includes('/load') || evt.request.url.includes('/session')) {
            //console.warn('[ServiceWorker] fetch backend LOAD => cacheDatabaseBackendLoad: ', evt.request.method, evt.request.url);
            cacheDataBackendLoad(evt);
            //network_or_cache(evt);
          } else {
            if (evt.request.url.includes('syncDownAll')) {
              //console.warn('[ServiceWorker] fetch caching: backend/syncDownAll disabled PASSTHRU: ', evt.request.url);
            } else {
              //console.log('[ServiceWorker] fetch backend NOT LOAD cacheData: ', evt.request.method, evt.request.url);
              cacheData(evt);
            }
          }
        }
      ...
      if (evt.request.url.includes('webpush.json') || evt.request.url.includes('quitteapp.nl') || evt.request.url.includes('.maptiler.') || evt.request.url.includes('/maps/') || evt.request.url.includes('/2/files/') || evt.request.url.includes('/fotos/') || evt.request.url.includes('/pois/') || evt.request.url.includes('/tracks/') || evt.request.url.includes('dropboxapi') || evt.request.url.includes('tiles') || evt.request.url.includes('.tile.') || evt.request.url.includes('.openstreetmap.')) {
...
function cacheDataBackendLoad(evt) {
  ...
  evt.respondWith(
    ...
    caches.match(evt.request, options).then(cacheRes => {
      if (cacheRes) {
...
function network_only(evt) {
  ...
  // or simply don't call evt.respondWith, which
