...
trinl.factory('dataFactoryAvatar', ['loDash', 'dataFactoryStore', 'dataFactoryAlive', function (loDash, dataFactoryStore, dataFactoryAlive) {

  //console.warn('dataFactoryAvatar');

  var dataFactoryAvatar = {};
  /**
   * me = this
   */
  var me = dataFactoryAvatar;
  /**
   * Naam store
   * @type {String}
   */
  ...
  me.Model = function (config) {
    ...
    if (config.avatarInverse === true) {
      this.avatarInverse.value = true;
    }
    ...
    unsetAll: function () {
      ...
      loDash.each(m, function (field) {
        if (field !== true && field !== false) {
          console.log('avatar unsetAll m, field: ', m, field);
          field.dirty = false;
        }
      });
      var Id = m.get('Id');
  ...
  dataFactoryAvatar.init = function () {
    //console.warn('dataFactoryAvatar init');
    dataFactoryAvatar.fsReady = false;
    dataFactoryAvatar.loaded = false;
    dataFactoryAvatar.star = [];
    dataFactoryAvatar.nieuw = [];
    dataFactoryAvatar.selected = [];
    dataFactoryAvatar.data = [];
    dataFactoryAvatar.store = [];
    dataFactoryAvatar.removedRecords = [];
    dataFactoryAvatar.current = '';
    dataFactoryAvatar.filters = [];
    dataFactoryAvatar.sorters = [];
    dataFactoryAvatar.actualTime = '1970-01-02 00:00:00';
    dataFactoryAvatar.lastSyncDate = '1970-01-02 00:00:00';
    dataFactoryAvatar.tmpArray = [];
  };

  return dataFactoryAvatar;
