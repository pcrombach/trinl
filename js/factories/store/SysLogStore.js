...
trinl.factory('SysLogStore', ['loDash', 'dataFactoryStore', 'dataFactoryObjectId', 'dataFactoryAlive',
  function (loDash, dataFactoryStore, dataFactoryObjectId, dataFactoryAlive) {

    //console.warn('SysLogStore');
    ...
    me.Model = function (config) {
      ...
      this.type = {
        value: config.type || 'client',
        dirty: false,
        type: 'string'
      };
      this.level = {
        value: config.level || 'log',
        dirty: false,
        type: 'string'
      };
      this.omschrijving = {
        value: config.omschrijving || '',
        dirty: false,
        type: 'string'
      };
      ...
      setAll: function () {
        ...
        loDash.each(me.data, function (item) {
          if (item.record.get('Id') === Id) {
            item.dirty = true;
            //console.log('setAll item: ', item);
            return false;
          }
        });
        //console.log('setAll velden: ' + JSON.stringify(m));
        return m;
      },
      unsetAll: function () {
