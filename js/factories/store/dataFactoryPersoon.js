...
trinl.factory('dataFactoryPersoon', ['loDash', '$interval', 'dataFactoryStore', 'dataFactoryAlive',
  function (loDash, $interval, dataFactoryStore, dataFactoryAlive) {

    //console.warn('dataFactoryPersoon');

    var dataFactoryPersoon = {};
    var me = dataFactoryPersoon;
    ...
    me.Model = function (config) {
      ...
      this.voorLetters = {
        value: config.voorLetters || '',
        dirty: false,
        type: 'string'
      };
      this.voorNaam = {
        value: config.voorNaam || '',
        dirty: false,
        type: 'string'
      };
      this.tussenVoegsel1 = {
        value: config.tussenVoegsel1 || '',
        dirty: false,
        type: 'string'
      };
      this.achterNaam = {
        value: config.achterNaam || '',
        dirty: false,
        type: 'string'
      };
      this.tussenVoegsel2 = {
        value: config.tussenVoegsel2 || '',
        dirty: false,
        type: 'string'
      };
      this.meisjesNaam = {
        value: config.meisjesNaam || '',
        dirty: false,
        type: 'string'
      };
      this.naam = {
      ...
      this.straatNaam = {
        value: config.straatNaam || '',
        dirty: false,
        type: 'string'
      };
      this.huisNummer = {
        value: config.huisNummer || '',
        dirty: false,
        type: 'string'
      };
      this.postCode = {
      ...
      if (config.xpriveAvatar === false) {
        this.xpriveAvatar.value = false;
      }
      this.xpriveAdres = {
        value: true,
        dirty: false,
      ...
      if (config.xprivedres === false) {
        this.xpriveAdres.value = false;
      }
      this.xpriveContact = {
        value: true,
        dirty: false,
      ...
      if (config.xpriveContact === false) {
        this.xpriveContact.value = false;
      }
      this.xpriveDiversen = {
        value: true,
        dirty: false,
      ...
      if (config.xpriveDiversen === false) {
        this.xpriveDiversen.value = false;
      }
      this.xprive = {
        value: true,
        dirty: false,
      ...
      unsetAll: function () {
        ...
        loDash.each(m, function (field) {
          if (field !== true && field !== false) {
            console.log('persoon unsetAll m, field: ', m, field);
            field.dirty = false;
          }
        });
        //            var Id = m.get('Id');
    ...
    me.save = function (model) {
      //console.error('Persoon.save: ', model);
    ...
    me.init = function () {
      //console.warn('dataFactoryPersoon init');
      me.fsReady = false;
      me.loaded = false;
      me.star = [];
      me.nieuw = [];
