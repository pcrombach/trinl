...
trinl.factory('dataFactoryGroepdeelnemers', ['loDash', '$interval', 'dataFactoryStore', 'dataFactoryAlive',
  function (loDash, $interval, dataFactoryStore, dataFactoryAlive) {

    //console.warn('dataFactoryGroepdeelnemers');

    var dataFactoryGroepdeelnemers = {};
    var me = dataFactoryGroepdeelnemers;

    me.storeId = 'groepdeelnemers';
    ...
    me.Model = function (config) {
      ...
      this.deelnemerId = {
        value: config.deelnemerId || '',
        dirty: false,
        type: 'string'
      };
      this.deelnemerNaam = {
        value: config.deelnemerNaam || '',
        dirty: false,
        type: 'string'
      };
      this.emailadres = {
      ...
      if (config.publicist === false) {
        this.publicist.value = false;
      }
      ...
      unsetAll: function () {
        ...
        loDash.each(m, function (field) {
          if (field !== true && field !== false) {
            console.log('groepdeelnemers unsetAll m, field: ', m, field);
            field.dirty = false;
          }
        });
        var Id = m.get('Id');
    ...
    me.clear = function () {
      return dataFactoryStore.clear(dataFactoryGroepdeelnemers);
    };

    me.clearFilter = function () {
      return dataFactoryStore.clearFilter(dataFactoryGroepdeelnemers);
    };

    me.clearSorter = function () {
      return dataFactoryStore.clearSorter(dataFactoryGroepdeelnemers);
    };

    me.count = function () {
      return dataFactoryStore.count(dataFactoryGroepdeelnemers);
    };
    ...
    me.first = function () {
      return dataFactoryStore.first(dataFactoryGroepdeelnemers);
    };
    ...
    me.getCount = function () {
      return dataFactoryStore.getCount(dataFactoryGroepdeelnemers);
    };

    me.getStoreId = function () {
      return dataFactoryStore.getStoreId(dataFactoryGroepdeelnemers);
    };
    ...
    me.initLoad = function () {
      return dataFactoryStore.initLoad(dataFactoryGroepdeelnemers);
    };

    me.isFiltered = function () {
      return dataFactoryStore.isFiltered(dataFactoryGroepdeelnemers);
    };

    me.isLoaded = function () {
      return dataFactoryStore.isLoaded(dataFactoryGroepdeelnemers);
    };

    me.isSorted = function () {
      return dataFactoryStore.isSorted(dataFactoryGroepdeelnemers);
    };

    me.last = function () {
      return dataFactoryStore.last(dataFactoryGroepdeelnemers);
    };
    ...
    me.nextPage = function () {
      return dataFactoryStore.nextPage(dataFactoryGroepdeelnemers);
    };

    me.previousPage = function () {
      return dataFactoryStore.previousPage(dataFactoryGroepdeelnemers);
    };
    ...
    me.storeInit = function () {
      return dataFactoryStore.storeInit(dataFactoryGroepdeelnemers);
    };
    ...
    me.sync = function () {
      return dataFactoryStore.syncUp(dataFactoryGroepdeelnemers)
        .then(dataFactoryStore.syncDown(dataFactoryGroepdeelnemers));
    };
    ...
    me.syncDown = function (update) {
      return dataFactoryStore.syncDown(dataFactoryGroepdeelnemers, update);
    };

    me.init = function () {
      //console.warn('dataFactoryGroepdeelnemers init');
      me.fsReady = false;
      me.loaded = false;
      me.star = [];
      me.nieuw = [];
      ...
      var interval = $interval(function () {
        if (localStorage.getItem('authentication_token') !== null) {
          ...
          me.storeInit().then(function () {
            var iedereen = loDash.filter(me.store, function (groepDeelnemerModel) {
              return groepDeelnemerModel.get('groep') === 'Iedereen';
            });
            //console.log(iedereen);
            var deelnemers = loDash.filter(me.store, function (groepDeelnemerModel) {
              return groepDeelnemerModel.get('groep') !== 'Iedereen';
            });
            //console.log(deelnemers)
            deelnemers = loDash.orderBy(deelnemers, o => o.get('groep'), 'asc');
            me.store = deelnemers.concat(iedereen);
            //loDash.each(me.store, function (groepDeelnemerModel) {
              //console.log('groepDeelnemers Groep, Naam: ', groepDeelnemerModel.get('groep'), groepDeelnemerModel.get('deelnemerNaam'));
            //})
            //console.log('dataFactoryGroepdeelnemers initieel loaded: ', me.store);
          });
        }
      }, 10, 200);
    };
    ...
    return dataFactoryGroepdeelnemers;
